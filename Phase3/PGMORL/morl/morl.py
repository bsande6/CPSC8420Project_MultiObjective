...
def run(args):
    ...
    while iteration < total_num_updates:
        ...
        if args.selection_method == 'moead':
            ...
            for i in range (1, len(last_fitness_scores)-1):
                for j in range(1, len(fitness_scores)-1):
                        if last_fitness_scores[i] > fitness_scores[j]:
                            elite_batch[j] = last_offspring_batch[i]
                            fitness_scores[j] = last_fitness_scores[i]
                            break
                        
        elif args.selection_method == 'random_mix':
          if episode % 5 != 4:
        ...
        elif args.selection_method == 'elite':
            
            elite_batch, scalarization_batch, predicted_offspring_objs, fitness_scores = population.elite_prediction_guided_selection(args, iteration, ep, opt_graph, scalarization_template)
            sorted_list = []
            sorted_objs = []
            length = len(elite_batch)
            for i in range (0, len(last_fitness_scores)):
              for j in range(0, len(last_fitness_scores)-1):
                if last_fitness_scores[j] < last_fitness_scores[j+1]:
                  temp = last_offspring_batch[j]
                  last_offspring_batch[j] = last_offspring_batch[j+1]
                  last_offspring_batch[j+1] = temp
                  temp = last_fitness_scores[j]
                  last_fitness_scores[j] = last_fitness_scores[j+1]
                  last_fitness_scores[j+1] = temp
            idx = np.argpartition(fitness_scores, int(len(elite_batch)/3))
            j = 0
            for i in range(0, int(len(last_fitness_scores)/3)):
              if (last_fitness_scores[i] > fitness_scores[idx[j]]):
                elite_batch[idx[j]] = last_offspring_batch[i]
                fitness_scores[idx[j]] = last_fitness_scores[i]
                j=j+1
