...
class CarRacing(gym.Env, utils.EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        if (temp is not None):
            action = [0,0,0]
            action[0] = temp[0][0].item()
            action[1] = temp[0][1].item()  
            action[2] = temp[0][2].item()     
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            self.fuel_reward -=  10 * self.car.fuel_spent / ENGINE_POWER
            self.car.fuel_spent = 0.0
            step_reward = self.reward - self.prev_reward
            step_fuel_reward = self.fuel_reward - self.prev_fuel_reward
            self.prev_reward = self.reward
            self.prev_fuel_reward = self.fuel_reward
        ...
        return self.state, 0, done, {'obj': np.array([step_reward, step_fuel_reward])}
